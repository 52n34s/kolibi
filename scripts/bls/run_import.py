#!/usr/bin/env python3
"""Import data/bls_import.sql into Postgres in a single transaction.

Copy of scripts/usda_fdc/run_import.py with BLS paths and counters.
Does not run unless you start it. Connection URL comes from SUPABASE_DB_URL
(set in the shell; do not commit it).

    pip install psycopg2-binary
    SUPABASE_DB_URL='postgresql://…' python3 scripts/bls/run_import.py
"""
...
SQL_PATH = ROOT / "data" / "bls_import.sql"
EXPECTED_BATCHES = 15  # 7140 rows / 500; USDA's copy is hardcoded to 17
COUNT_SQL = "SELECT count(*) FROM public.foods WHERE source = 'bls_4_0'"
...
def main() -> None:
    ...
    try:
        ...
        for batch_id, total, n_rows, sql in batches:
            ...
            (bls_count,) = cur.fetchone()
            print(
                f"batch {batch_id}/{total}, {n_rows} rows "
                f"(foods source=bls_4_0 count={bls_count})"
            )
