...
type Ranked = Recommendation & { rank: number };
 ...
 * nutrition the goal's focus order decides; dismissed kinds stay away for
 * three days.
 */
export function buildRecommendations(ctx: RecommendationContext): Recommendation[] {
  ...
  all.sort(
    (a, b) => CATEGORY_ORDER[a.category] - CATEGORY_ORDER[b.category] || a.rank - b.rank,
  );
