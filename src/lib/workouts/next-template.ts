 ...
 * 2) Else the unit after the most recently finished one still in the plan
 *    (by position, cyclic)
...
export function pickNextTemplate(
  ...
  // The newest session whose unit is still in the plan: a unit archived
  // right after its session must not reset the rotation to the first one.
  for (const session of finished) {
    const index = ordered.findIndex((template) => template.id === session.templateId);
    if (index >= 0) {
